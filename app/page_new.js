...
export default function App() {
  ...
  const fetchCryptos = useCallback(async (showRefreshing = false) => {
    try {
      ...
      if (result.isMockData) {
        setIsLiveData(false);
        toast.warning('⚠️ Live data temporarily unavailable. Showing mock data.', {
          duration: 5000,
        });
      } else {
        setIsLiveData(true);
  ...
  // Filter and sort cryptos based on current view
  useEffect(() => {
    let result = [...cryptos];
  ...
  const renderContent = () => {
    ...
    // Home, Gainers, and Watchlist views
    return (
      <>
        {/* Controls */}
        ...
        {!isLiveData && (
          <Alert className=\"mb-6 border-yellow-500/50 bg-yellow-500/10\">
            ...
            <AlertDescription className=\"text-yellow-700 dark:text-yellow-300\">
              ⚠️ Live data temporarily unavailable. Showing mock data. Data will auto-refresh when API is available.
            </AlertDescription>
          </Alert>
        )}
